...
function render({ posts, pages }, templateCache, outputDir) {
  ...
  // others
  const otherPages = Object.keys(templateCache.pages).map(name => {
    const url = path.join('/', name.replace(/(\/|^)index.html$/i, ''));
    const absoluteUrl = config.site.url + url;
    const page = pages[basename(name)] || pages[name] || {};
    return {
      file: path.join(outputDir, name),
      content: templateCache.pages[name].render({ url, absoluteUrl, page })
    };
  });
