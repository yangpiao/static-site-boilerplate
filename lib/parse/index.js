...
const { readAll, crossReference } = require('../utils');
...
async function parse(dir) {
  ...
  pageFiles.forEach(file => {
    pageCache[file.path.relative] = parsePage(file);
  });
  ...
  pageFiles.forEach(file => {
    Object.assign(pageCache[file.path.relative], parseContent(file));
  });
...
function processPosts(postCache) {
  if (!config.blog) return postCache;
  const tags = {};
